...
bottom_up_country_options <- tibble(
  country = c("Australia", "New Zealand", "Canada", "China",
              "India", "Japan", "South Korea", "Mexico",
              "Germany", "United Kingdom"),
  rcp_26_region = c("Australia/New Zealand", "Australia/New Zealand",
                    "Canada", "China", "India", "Japan", "South Korea",
                    "Latin America", "Western Europe", "Western Europe")
)
...
ref_year <- unique(rcp_26$ref_year)
target_year <- unique(rcp_26$target_year)
if (length(ref_year) != 1 || length(target_year) != 1) {
  stop("ERROR: the RCP 2.5 reference and target years are not unique.")
}
...
* "**Calculations**" shows the steps for you to follow for the country you have
  chosen
...
For this assignment, analyze the economy and carbon emissions from a country 
that you will choose.

## Decarbonization Lab, due `r long_due_date_1`

For this lab, you will choose one of the following countries and
perform a bottom-up analysis of decarbonization policy for that country: 
`r head(bottom_up_country_options$country, -1) %>% str_c(collapse = ", ") %>%
  str_c(", or ", tail(bottom_up_country_options$country, 1))`.
...
#   kable(align = "ll")
#
# bottom_up_country_options %>% 
#   left_join(rcp_26, by = c("rcp_26_region" = "region")) %>%
#   select(country, reduction) %>%
#   mutate(reduction = scales::label_percent(accuracy = 1)) %>%
#   rename(Country = country, "Target reduction" =)
#   kable(align = "lr")
```
...
To analyze the policy for your country:

#. Get the Kaya identity data for the country
#. Figure out appropriate starting years for calculating the historical trends 
   for the Kaya variables _P_, _g_, _e_, and _f_.
#. Comment on whether you think it's reasonable to extrapolate the historical
   trend.
#. Calculate the _historical trends_ for the Kaya variables from the starting
   year you determined in step (2).
#. Use the _historical trends_ to extrapolate projected values for _P_, _g_, 
   _e_, and _f_ in `r target_year`.
#. Calculate the policy goal for emissions _F_ in `r target_year`.
   ...
   country.
#. Calculate the _implied rate of change_ of _F_ between `r current_year`
   and `r target_year`, in order to reduce emissions to the policy goal that
   you calculated in step (6).
#. Combine the _implied rate of change_ of _F_ with the _historical trends_ of
   _P_ and _g_ to calculate the _implied rate of change_ of _ef_ that you 
   calculated in step (4) in order to meet the policy goal from step (6).
#. Compare the _implied rate of change_ of _ef_ that you calculated in step (8)
   to the _historical trend_ of _ef_ that you can determine from the
   _historical trends_ of _e_ and _f_ that you calculated in step (4).

### Detailed steps:

#. Use the `kayadata` package in RStudio to load the data for
   your country. Below is an example of looking up the data
   for the United States:
   ```{r get_usa_data, echo=TRUE, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
   library(kayadata)
   usa_data = get_kaya_data("United States")
   latest_year = max(usa_data$year)
   usa_current = usa_data %>% filter(year == latest_year)
   ```
   You can get a list of all the countries and regions that are available 
   from the function `kaya_region_list()`.

#. Next, go to the "Trends" tab in the interactive application at 
   <https://ees3310.jgilligan.org/decarbonization> and look at the graphs of 
   ...
   * Is there a better starting year for calculating trends? 
     ...
     (you may use different start years for different variables).
   * Do you anticipate a problem if we make policy by assuming that 
     ...
     You should also plot each variable in your report using RMarkdown. 
     Following from the example above, you can use the `plot_kaya` function:
     ```{r plot_usa_data, echo=TRUE, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
         plot_kaya(usa_data, "e", log_scale = TRUE, start_year = 1990, 
                   trend_line = TRUE,  trend_color = "red", font_size = 12)
     ```
...
#. Examine the plot for each variable. 
   ...
   do the data lie close to the trend line you plotted?
   
   Would you be confident predicting that the changes over the next 30 years 
   will continue to follow the trend line in your plot?
   
   Explain why you either are or are not confident in extrapolating the data 
   using the trend lines in the plots.

#. Next, calculate the rates of change of 
   ...
   Here is an example of calculating the rates of change of _P_ and _e_ 
   (the energy intensity of the economy) for the United States, 
   using the variable `usa_data` that you calculated above:
   ```{r get_usa_e_trend, echo=TRUE, include=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
...
#. Using the rates of change that you determined in the previous step, 
...
#. Calculate the emissions target for the country you've chosen: 
   ...
   bottom_up_country_options %>% 
     left_join(rcp_26, by = c("rcp_26_region" = "region")) %>%
     mutate(reduction = scales::label_percent(accuracy=1)(reduction)) %>%
     select(Country = country, "Target reduction" = reduction) %>%
     kable(caption = str_c("Percent reduction in CO~2~ emissions in ", target_year,
                           ", relative to ", ref_year, "."),
           align = "lr")
   ```
   ...
   How much CO~2~ (_F_) would the country you're analyzing emit in
   `r target_year` in order to meet its policy goal?
    
   Let's work an example using Argentina:
   ```{r argentina_example, echo = TRUE, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
   F_2005_argentina = get_kaya_data("Argentina") %>% 
     filter(year == 2005) %$% F
   F_2005_argentina
   ```
   ```{r argentina_example_2, echo = TRUE, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
   argentina_reduction = 0.40
...
6. Look up what the CO~2~ emission is in `r current_year` and calculate the 
   ...
   For our Argentina example:
   ```{r argentina_example_3, echo = TRUE, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
   F_2017_argentina = get_kaya_data("Argentina") %>% 
     filter(year == 2017) %$% F
   r_F_argentina = log(F_goal_argentina / F_2017_argentina) / (2050 - 2017)
   r_F_argentina
   ```
   so total emissions for the Argentina would need to drop by 
   `r round(100 * r_F_argentina, 2)`% per year between `r current_year`
   and `r target_year`
...
7. Now calculate the decarbonization rate implied by the policy goal. This is 
   ...
   The rate of decarbonization has the opposite sign to the rate of change of
   _ef_: If the economy is decarbonizing by 20% per year, that means that the 
   rate of change of _ef_ is $-0.20$.
   
   Multiply the rate of change of _ef_ by -1 to get the rate of decarbonization
   and multiply by 100 to express it as a percentage change per year.

8. How does the implied rate of change of _ef_ for the country you're analyzing
   compare to the 
   historical rate (i.e., the trend in _ef_ reported in the 
   "Bottom up Analysis" table)? 
   Do you think your country will have a hard time meeting its emission goal 
   without damaging its economy?
