...
* `run_rrtm()` allows you to automatically download a file
  ...
  * `lapse_rate` is the lapse rate, in Kelvin per kilometer. 
    The default is 6. 
...
You can also leave out the `file` argument to `run_modtran` if you don't need
...
like `foo`. Suppose this function `foo` returns a number
and I want my R code to behave differently if `foo(x)` > x than if 
...
function, which I will call `compare_f`:
```{r compare_foo}
...
print(str_c("foo(", x, ") is ", compare_f(foo, x), " ", x))
```
```{r compare_foo_greater}
x = 2.0
print(str_c("foo(", x, ") is ", compare_f(foo, x), " ", x))
```
You can also incorporate this output into your RMarkdown text.
The RMarkdown text 
````markdown
When x = 0.5 foo(x) is ``r "r"` compare_f(foo, 0.5)` x, 
and when x = 1.0 foo(x) is ``r "r"` compare_f(foo, 1.0)` x,
and when x = 2.0 foo(x) is ``r "r"` compare_f(foo, 2.0)` x.
````
...
a) Run the MODTRAN model with present-day CO~2~ (400 ppm) and a tropical 
   atmosphere. 
...
b) Now set `atmosphere` to `"midlatitude winter"`, set `clouds` to `"none"`,
   ...
   * For the atmosphere with no water vapor, compare the parts of the 
     ...
     * Which corresponds to higher emission temperatures and which to lower 
       temperatures? 
