...
values for the CO~2~ concentration:

```{r modtran_loop, include=TRUE, message=FALSE, warning=FALSE}
...
b) Where in the spectrum does methane absorb? What concentration does it take to
  ...
  * Run MODTRAN several times, successively doubling the amount of methane:
    ...
    save them in the `"_data"` folder as `"methane_0_4.txt"`, 
    `"methane_0_8.txt"`, `"methane_1_6.txt"`, and so forth, up to 
    `"methane_819_2.txt"`, and also create a data tibble `methane_data` 
    with a list of methane concentrations and the corresponding 
    $I_{\text{out}}$.
...
a) Run the MODTRAN model in using the "Tropical" atmosphere, without clouds, 
   and with present-day pCO~2~ (400 ppm). Use the ground temperature reported 
   by the model to calculate
   $\varepsilon \sigma T_{\text{ground}}^4$, the heat flux emitted by the 
   ground.
