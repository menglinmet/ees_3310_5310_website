...
due_date_1 <- date("2020-04-16")
due_date_2 <- date("2020-04-20")
...
top_down_country_options <- tibble(
...
infrastructure you would need to install for the country you analyzed in 
the bottom-up lab, in order to meet the emissions-reduction goals 
for `r target_year`.

You will analyze the same country you did in the bottom-up lab last 
week, from the options of 
`r head(top_down_country_options$country, -1) %>% str_c(collapse = ", ") %>%
  str_c(", or ", tail(top_down_country_options$country, 1))`.
```{r country_table}
# top_down_country_assignments %>% group_by(class) %>%
#   summarize(regions = str_c(region, collapse = ", ")) %>%
#   kable(align = "ll")
#
# top_down_country_options %>% 
...
The lab report is due (knitted to PDF document, committed, and pushed
to GitHub) by the end of the day `r long_due_date_2`

# The Assignment:

## Outline:

1. For your country, examine the top-down projections for the Kaya 
...
8. Suppose instead that the country supplied its growing energy 
...
1. Use the function `project_top_down()` from the 
...
3. If the  country uses the same percentages of each energy source in 
   ...
   **Hint:** You can combine data frames using `join` functions to make this
   easier. Here is an example, which I have worked for France. You can
   do the same thing for other countries:
   ```{r example_fuel_mix}
   E_2050 <- get_top_down_values("France") %>% 
     filter(year == 2050) %$% E
   fm <- get_fuel_mix("France")
   ...
   factors <- emissions_factors() %>% 
     mutate(fuel = lvls_expand(fuel, new_levels = levels(fm_2050$fuel)))
