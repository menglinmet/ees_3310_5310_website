...
v_water <- 321003271 # cubic miles of water, from http://oceanservice.noaa.gov/facts/oceanwater.html
cubic_mile <- (5280. * 12. * 2.54 / 100)^3 #cubic mile to cubic meter
n_water <- v_water * cubic_mile * 1E+6 / (12 + 32) * 6.02E+23 # 1 million grams per cubic meter
# 12 + 32 grams per mole, 6.02E+23 molecules per mole.
```
...
```{r standard_modtran, echo=FALSE, include=FALSE, eval=TRUE, cache=TRUE}
modtran_std <- run_modtran(atmosphere = "standard")
modtran_tropical <- run_modtran(atmosphere = "tropical")
modtran_midlat_winter <- run_modtran(atmosphere = "midlatitude winter")
```
...
* {+} Things that don't change during a run:
  ...
  * Temperature of the ground and every layer of the atmosphere.
    * Set by "locality" of the atmosphere and "temperature offset"
      ...
      |:------:|----------------:|--------------:|
      | U.S. Standard Atmosphere | `r prt(modtran_std$i_out, 2)` | `r prt(modtran_std$t_ground, 1)` |
      | Tropical | `r prt(modtran_tropical$i_out, 2)` | `r prt(modtran_tropical$t_ground, 1)` |
      | Midlatitude winter | `r prt(modtran_midlat_winter$i_out, 2)` | `r prt(modtran_midlat_winter$t_ground, 1)` |
      :::
...
```{r dryas, echo=FALSE, cache=TRUE}
gisp2 <- read.table(file.path(data_dir, 'paleo', 'gisp2_temp_accum_alley2000.txt'), 
                    header=F, skip=75, stringsAsFactors=F, nrows=1632, 
                    col.names=c('age','t'))
dryas <- data.frame(start=11.6,stop=14.3)

ggplot(gisp2[gisp2$age <=40,],aes(x=age,y=t)) + 
  geom_line(size=I(1)) + 
  scale_x_reverse() + 
  scale_y_continuous(limits = c(-55,-25), expand = c(0,0)) +
  labs(x="Thousand years before present", 
       y=expression(paste("Central Greenland Temp ", (degree * C)))) + 
  theme_classic(base_size=30)
```

## Abrupt Climate Change  {#abrupt-2 data-transition="fade-in"}

```{r dryas_2, echo=FALSE, warning=FALSE, cache=TRUE}
ggplot(gisp2[gisp2$age <=40,],aes(x=age,y=t)) + 
  geom_line(size=I(1)) + 
  geom_rect(aes(NULL,NULL,xmin=start,xmax=stop), ymin=-55, ymax=-25, data=dryas, 
            fill='blue', alpha= 0.1) + 
  scale_x_reverse() + 
  scale_y_continuous(limits = c(-55,-25), expand = c(0,0)) +
  labs(x="Thousand years before present", 
       y=expression(paste("Central Greenland Temp ", (degree * C)))) + 
  annotate("text",x=15,y=-30,vjust=0,hjust=1,label="Younger Dryas", size=10, 
           color='darkblue') +
  theme_classic(base_size=30)
```
