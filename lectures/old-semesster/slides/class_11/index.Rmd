...
```{r solubility, include=FALSE, message=FALSE}
t <- seq(-5,40,0.3)
...
```{r solubility_chart, echo=FALSE, fig.width=950/72, fig.height=750/72, dpi=72}
p <- ggplot(solubility, aes(x=t, y=c)) + geom_line(size=I(1), color=I('dark blue')) +
  scale_y_continuous(limits=c(0,max(solubility$c))) +
  labs(x="Temperature (C)", y=expression(paste(CO[2], " (grams per 100 ml)")),
       title = expression(paste("Solubility of ", CO[2], " in water"))) +
...
::::::
:::::: {.column .bare}
![thermocline](assets/images/thermocline.png){style="height:900px;"}
...
```{r co2_spike, echo=FALSE, warning=FALSE, cache.extra=c(file.info(file.path(data_dir,'carbon_cycle','co2spike.txt'))$mtime)}
co2.spike <- read.table(file.path(data_dir, "carbon_cycle", "co2spike.txt"), 
                        header=T, quote="\"")

label <- 'paste(CO[2], " equilibration takes millennia")'

p <- ggplot(co2.spike, aes(x=year/1000, y=pCO2)) + 
  geom_line(size=I(1), color=I('dark blue')) +
  labs(x = 'Thousand years', y=expression(paste(CO[2], ' (ppm)'))) +
  annotate('text', color=I('dark blue'),
           label=label, parse=T,
            x=5,y=1000, size=15) +
