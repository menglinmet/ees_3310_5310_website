...
```{r textbooks, echo=F, eval=T, message=F, warning=F}
library(pander)
textbooks <- data_frame(image = file.path('assets/images',
                                            c('understanding_forecast.jpg',
                                              'climate_casino.jpg',
                                              'climate_fix.jpg',
                                              'r4ds.png')),
  author = c('David Archer',
           'William Nordhaus',
           'Roger A. Pielke, Jr.',
           'Hadley Wickham & Garrett Grolemund'),
  title = c('Global Warming: Understanding the Forecast (2<sup>nd</sup>&nbsp;Edition)',
           'The Climate Casino',
           'The Climate Fix',
           'R for Data Science'),
  supp = c("", "", "", 
           "<br/>(Free web version online at <a href=\"http://r4ds.had.co.nz/\" target=\"_blank\"><code>http://r4ds.had.co.nz/</code></a>)"))
```
<table>
```{r textbook_table, echo=F, eval=T, message=F, warning=F, results='asis', dependson=c('textbooks')}
library(yaml)
library(rvest)
strip_html <- function(s) html_text(read_html(paste0("<html>", s, "</html>")))
output_params <- yaml.load_file("_output.yml")[["revealjg::revealjs_presentation"]]
img_height <- paste0(as.character(output_params$height/7),'px')
message("img_height = ", img_height)
rows <- c()
for(i in seq_along(textbooks$image)) {
  row <- '<tr style="vertical-align:middle; text-align:center">'
  row <- with(textbooks[i,], paste0(row,'<td><img src="', image, 
                                    '" height="', img_height, 
                                    '" alt="', strip_html(title), '"/></td>'))
  row <- with(textbooks[i,], paste0(row,'<td style="vertical-align:middle;">',author,', <i>', title, '</i>', supp, '</td>'))
  row <- paste0(row, '</tr>')
  rows <- c(rows, row)
}
writeLines(rows)
...
```{r giss, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, cache.extra=list(data_dir,script_dir,file.info(file.path(data_dir, 'global_temp', 'giss', 'GLB.Ts+dSST.txt'))$mtime, file.info(file.path(script_dir, 'knit_giss_monthly.R'))$mtime, file.info(file.path(script_dir, 'load_giss.R'))$mtime, file.info(file.path(script_dir, 'download_giss_data.R'))$mtime)}
source_semester_script('load_giss.R')
source_semester_script('knit_giss_monthly.R')
giss_data <- load_giss_data('land.sea')
make_gisplot(giss_data, n_warmest = 10, baseline_years = min(giss_data$data$year) + c(1,30) - 1, inc_last_month = FALSE, base_size = 20, warm_row = c(5,5))
```

<!--- dummmy: Keeling curve --->      {data-transition="fade-out"}
------

```{r keeling, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, cache.extra = list(data_dir,script_dir,file.info(file.path(data_dir, 'keeling', 'co2_mm_mlo.txt'))$mtime, file.info(file.path(script_dir, 'load_keeling.R'))$mtime, file.info(file.path(script_dir, 'knit_keeling.R'))$mtime, file.info(file.path(script_dir, 'download_keeling_data.R'))$mtime)}
source_semester_script('load_keeling.R')
source_semester_script('knit_keeling.R')
co2_data <- get.co2(data_dir)
plot.co2(co2_data, inset = FALSE, base_size = 20)
```

## Past 2000 Years     {data-transition="slide"}

```{r paleo_2000, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE, fig.height = 7.75, cache.extra=list(data_dir,script_dir,file.info(file.path(data_dir, 'paleo', 'law2006.txt'))$mtime, file.info(file.path(script_dir, 'paleo_co2.R'))$mtime)}
source_semester_script('paleo_co2.R')
law_co2 <- load_law_dome_co2(data_dir = data_dir)
law_plot <- plot_law_dome(law_co2)
law_plot + ylim(250,410)
```

## 800,000 years of CO~2~ {data-transition="fade-out"}

```{r paleo_800k, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE, fig.height = 7.75, cache.extra=list(data_dir,script_dir,file.info(file.path(data_dir, 'paleo', 'edc-co2-2008.txt'))$mtime, file.info(file.path(script_dir, 'paleo_co2.R'))$mtime)}
