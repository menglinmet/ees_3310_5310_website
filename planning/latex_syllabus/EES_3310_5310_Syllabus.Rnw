...
semester <- load_semester_db("../EES_3310_5310.sqlite3")
...
reading <- semester$rd_items %>%
  select(rd_item_id, rd_key, title = short_latex_title, chapter, pages,
         textbook, handout, optional, undergraduate_only, graduate_only,
         break_before) %>%
  format_reading_item() %>% filter(! is.na(reading)) %>%
  arrange(rd_key, handout, optional, graduate_only, undergraduate_only) %>%
  group_by(rd_key, handout) %>%
  summarize(reading = collapse_handouts(reading, handout)) %>%
  ungroup() %>%
  arrange(rd_key, handout) %>% group_by(rd_key) %>%
...
calendar <- schedule %>%
  left_join(reading, by = c(key_class = "rd_key")) %>%
...
<<r grading, include=F>>=
grade_weights <- c(participation = 5, homework = 0, midterm = 25, lab = 45, final = 25)
...
scientific ommunity. Indeed, the mathematician and poet Jacob Bronowski wrote,
...
schedule_break_row <- with(schedule, which(month == "Feb." & day == 28))
